...
Windows stable and Unified Avalonia Preview have separate release paths. Unified Avalonia Preview `0.3.1` ships synchronized Windows x64 and Apple Silicon macOS ZIPs from the same prerelease. The macOS artifact is Developer ID signed, notarized, stapled, and verified with Gatekeeper. The Windows artifact is a self-contained Windows x64 ZIP and is separate from the stable Windows WPF ZIP/release path.
