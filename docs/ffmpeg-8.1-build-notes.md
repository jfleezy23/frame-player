...
For the current `v1.4.4` GPU release, use `docs\gpu-vulkan-phase1-release.md` as the maintainer-facing product summary and keep this file as the runtime/build provenance source of truth.
...
- If neither local source exists, the script downloads the pinned runtime from the verified `v1.4.4` release asset declared in the manifest and validates both the archive SHA256 and extracted DLL hashes.
...
This is intentional and now supported. Clean GitHub runners do not have staged local FFmpeg runtime artifacts, so the workflow restores the runtime from the verified `v1.4.4` release asset recorded in `Runtime\runtime-manifest.json`.
...
- If the runtime asset ever moves off the `v1.4.4` release, update both `tag`/`assetUrl` and CI expectations in the same change.
