...
- Start with a conservative shared-library build rather than a heavily minimized `--disable-everything` build. The app opens user-provided `.avi`, `.mov`, `.m4v`, `.mp4`, `.mkv`, `.wmv`, and `.ts` files, so over-pruning demuxers/decoders is a higher release risk than shipping a moderately broader native runtime.
