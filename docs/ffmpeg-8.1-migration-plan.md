...
This is a historical migration checkpoint. The FFmpeg 8.1 runtime is now the active runtime in the repo. Keep `docs\ffmpeg-8.1-build-notes.md` as the current runtime provenance note and `docs\release-v1.6.0-loop-export.md` as the current branch product/release note.
