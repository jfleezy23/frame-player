...
This is a historical planning checkpoint. The current WPF app remains the working reference implementation for `v1.4.2`, but current shipped behavior is documented in `docs\gpu-vulkan-phase1-release.md`. This note exists to preserve the earlier extraction plan, not to define the active release surface.
