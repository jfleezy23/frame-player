...
{
    internal sealed class DesktopRecentFilesService
    {
        private const int MaxRecentFiles = 10;
        private readonly string _storagePath;

        public DesktopRecentFilesService()
        {
            var appSupport = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            var appDirectory = Path.Combine(appSupport, "FramePlayer.DesktopPreview");
            Directory.CreateDirectory(appDirectory);
            _storagePath = Path.Combine(appDirectory, "recent-files.txt");
        }

        internal DesktopRecentFilesService(string storagePath)
        {
