...
{
    public sealed class DesktopRecentFilesServiceTests
    {
        [Fact]
        public void Add_KeepsMostRecentExistingFilesWithoutDuplicates()
        {
            ...
            try
            {
                ...
                var service = new DesktopRecentFilesService(storagePath);
        ...
        public void Add_LimitsRecentFilesToTenEntries()
        {
            ...
            try
            {
                var service = new DesktopRecentFilesService(Path.Combine(root, "recent-files.txt"));
        ...
        {
            var path = Path.Combine(Path.GetTempPath(), "frame-player-desktop-tests-" + Guid.NewGuid().ToString("N"));
