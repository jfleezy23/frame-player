...
$resolvedWorkRoot = if ([string]::IsNullOrWhiteSpace($WorkRoot)) {
    Join-Path $env:TEMP "frameplayer-ffmpeg-8.1.2-source-build"
}
else {
    $WorkRoot
}
$resolvedCandidateRuntimePath = if ([string]::IsNullOrWhiteSpace($CandidateRuntimePath)) {
    Join-Path $repoRoot "Runtime\ffmpeg-8.1.2-candidate"
}
else {
...
echo "Candidate FFmpeg 8.1.2 runtime staged at: `$candidate_dir"
ls -la "`$candidate_dir"
"@

$temporaryScriptPath = Join-Path $env:TEMP ("frameplayer-build-ffmpeg-8.1.2-" + [Guid]::NewGuid().ToString("N") + ".sh")
