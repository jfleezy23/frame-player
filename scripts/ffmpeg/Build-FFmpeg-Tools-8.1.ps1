...
$resolvedWorkRoot = if ([string]::IsNullOrWhiteSpace($WorkRoot)) {
    Join-Path $env:TEMP "frameplayer-ffmpeg-tools-8.1.2-source-build"
}
else {
    $WorkRoot
}
$resolvedCandidateToolsPath = if ([string]::IsNullOrWhiteSpace($CandidateToolsPath)) {
    Join-Path $repoRoot "Runtime\ffmpeg-tools-8.1.2-candidate"
}
else {
    $CandidateToolsPath
}
...
$temporaryScriptPath = Join-Path $env:TEMP ("frameplayer-build-ffmpeg-tools-8.1.2-" + [Guid]::NewGuid().ToString("N") + ".sh")
