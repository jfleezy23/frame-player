...
Replace `<release-version>` with the exact candidate or final version being packaged. Release-candidate corpus tests are tagged `Category=ReleaseCandidate` and require the platform-specific corpus environment documented in `docs/macos-release.md` or the Windows Rust corpus harness.
